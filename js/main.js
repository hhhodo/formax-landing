  ...
  const onFrame = () => {
    ...
    if (stackLineArt && ctaCard) {
      if (stackedMedia.matches) {
        // stacked layout: the photo is a plain block BELOW the CTA text (not
        // pinned/overlapping it), so the wipe is driven by how far the photo
        // itself has scrolled through the viewport.
